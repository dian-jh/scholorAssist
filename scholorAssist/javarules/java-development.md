...
# Java开发规范 - 文献辅助阅读系统
## 本项目技术栈
本项目是一个文献辅助阅读系统，采用微服务架构。使用Spring Boot + MyBatis-plus + PostgreSQL技术栈。
## 核心原则
- 严格遵循 **SOLID、DRY、KISS、YAGNI** 原则
- 遵循 **OWASP 安全最佳实践**（输入验证、SQL注入防护）
- 采用 **微服务分层架构设计**，确保职责分离和模块独立
- 代码变更需通过 **单元测试覆盖**（测试覆盖率 ≥ 80%）
- 每次写完所有的代码都要进行build操作确认是否有语法错误
## 微服务架构规范

### 模块划分
- **[sc-common](mdc:sc-common)**: 通用工具模块（工具函数、枚举类、通用DTO、异常定义、公共配置）
- **[sc-gateway](mdc:sc-gateway)**: API网关模块（路由转发、认证鉴权、限流熔断、统一响应）
- **[sc-user-service](mdc:sc-user-service)**: 用户服务模块（用户注册、登录、信息管理、会话管理）
- **[sc-categories-manage](mdc:sc-categories-manage)**: 分类管理服务（文档分类的CRUD、层级管理）
- **[sc-literature-manage](mdc:sc-literature-manage)**: 文献管理服务（PDF上传、解析、存储、检索）
- **[sc-note-service](mdc:sc-note-service)**: 笔记服务模块（笔记CRUD、标签管理、关联查询）
- **[sc-ai-service](mdc:sc-ai-service)**: AI服务模块（对话管理、模型调用、向量检索）
- **[sc-statistics-service](mdc:sc-statistics-service)**: 统计服务模块（用户行为统计、数据分析）

### 分层职责规范

#### Gateway层 (`sc-gateway`)
- **职责边界**: 仅负责API路由、认证鉴权、参数校验和请求转发，**禁止编写业务逻辑**
- **调用规范**: 通过Feign客户端调用各微服务的Service接口
- **API规范**: 
  - 请求方法：支持GET、POST、PUT、DELETE请求
  - 参数规范：超过2个参数必须定义DTO实体
  - 响应格式：统一使用 [Result](mdc:sc-common/src/main/java/com/scholar/common/result/Result.java) 封装
- **路径规范**: 
  - 用户管理: `/api/users/`
  - 分类管理: `/api/categories/`
  - 文档管理: `/api/documents/`
...
- **微服务通信**: 通过Feign客户端进行服务间调用，避免直接数据库跨服务访问

#### Manager层 (各微服务模块)
- **职责**: 封装数据库访问与缓存操作，聚合多表读写，提供细粒度的领域方法
- **调用规范**: 仅供 Service 层调用，禁止 Gateway 层或其他层直接调用
- **资源访问**:
  - 只能调用本服务内的 Mapper 接口进行数据库操作
  - 统一负责 Redis 缓存的读写，必须使用 [RedisUtil](mdc:sc-common/src/main/java/com/scholar/common/redis/RedisUtil.java)
- **设计原则**: 一个领域聚合一个 Manager，可组合调用其他 Manager，但应避免循环依赖
- **事务规范**: 跨表写操作必须使用 `@Transactional(rollbackFor = Exception.class)`

#### Mapper层 (各微服务模块)
- **职责**: 数据库访问接口定义，SQL映射配置
- **文件位置**: 接口在 `src/main/java/.../mapper/`，XML在 `src/main/resources/mapper/`
- **命名规范**: 接口以Mapper结尾，XML文件与接口同名
- **SQL规范**: 所有SQL语句必须写在XML文件中，禁止使用注解SQL
